...
- [x] 1.2 Deploy to homelab (`just a homelab` — may fail on nix-secrets fetch, that's expected; verify the sudoers line lands in `/etc/sudoers.d/`)
...
- [x] 2.2 Activate on MBP (`just a`) and verify `~/.ssh/config` contains `ForwardAgent yes` for `fedorivns-homelab.local`
...
- [x] 4.1 Run `just a homelab` from the MBP; confirm the deploy completes successfully (nix-secrets fetched, system activated)
- [x] 4.2 On the homelab, run `sudo env | grep SSH_AUTH_SOCK` during an active SSH session from MBP to confirm the socket is preserved under sudo
